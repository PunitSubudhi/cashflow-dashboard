...
def parse_bank_statement(
    ...
    try:
        ...
        if file_name.endswith(".csv"):
            ...
            return (
                None,
                f"Unsupported file format: {file_name}. Use CSV or Excel files.",
            )
        ...
        for target, source in [
            ...
            if not matched:
                return (
                    None,
                    f"Could not find column '{target}'. Available columns: {available_columns}",
                )
        ...
        if amount_col:
            ...
            if amount_series.dtype == object:
                ...
                amount_series = amount_series.str.replace(
                    r"\(([^)]+)\)", r"-\1", regex=True
                )  # Handle (100) as -100
        ...
        if normalized.empty:
            return (
                None,
                "No valid data found after parsing. Check date and amount formats.",
            )
...
def detect_columns(file_content: bytes, file_name: str) -> Dict[str, List[str]]:
    ...
    try:
        ...
        date_candidates = [
            c for c in columns if any(k in c.lower() for k in date_keywords)
        ]

        # Detect description columns
        desc_keywords = ["description", "memo", "narrative", "details", "payee", "name"]
        desc_candidates = [
            c for c in columns if any(k in c.lower() for k in desc_keywords)
        ]

        # Detect amount columns
        amount_keywords = ["amount", "value", "debit", "credit", "sum", "total"]
        amount_candidates = [
            c for c in columns if any(k in c.lower() for k in amount_keywords)
        ]

        return {
            "all_columns": columns,
            "date_candidates": date_candidates or columns[:1],
            "description_candidates": desc_candidates or columns[1:2]
            if len(columns) > 1
            else [],
