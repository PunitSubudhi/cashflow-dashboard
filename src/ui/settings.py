...
def render():
    ...
    with settings_col2:
        ...
        st.metric("Opening Date", opening_date.strftime('%Y-%m-%d') if opening_date else "Not set")
    ...
    if categories:
        ...
        st.info("No categories defined yet. Categories are created automatically when you add transactions.")

    st.divider()
    
    # Data Portability Section
    st.subheader("📦 Data Export & Import")
    st.write("Export all your data to a file for backup or to transfer to another instance of the app.")
    ...
    with import_col:
        ...
        if uploaded_file is not None:
            st.warning("⚠️ Importing will **replace all existing data**. Make sure to export your current data first if needed.")
            ...
            if st.button("🔄 Import Data", type="primary"):
                try:
                    ...
                    st.write(f"- Recurrence Rules: {result['counts']['recurrence_rules']}")
