...
def render():
    ...
    if not cashflow_data:
        st.info("No transactions found. Add transactions or set up recurrence rules to see your cashflow.")
        return
    ...
    display_df = pd.DataFrame({
        ...
        'Inflow': df['inflow'].apply(lambda x: f"£{x:,.2f}" if pd.notna(x) else '-'),
        'Outflow': df['outflow'].apply(lambda x: f"£{x:,.2f}" if pd.notna(x) else '-'),
        'Status': df['status'],
    ...
    final_balance = cashflow_data[-1]['running_balance'] if cashflow_data else opening_balance
    ...
    mask = display_df['Status'].isin(status_filter) | (display_df['Description'] == '📍 Opening Balance')
    if not show_opening:
    ...
    # Display the table
    st.subheader(f"Cashflow Statement ({opening_date.strftime('%d %b %Y')} - {default_end.strftime('%d %b %Y')})")
    st.dataframe(
        filtered_df,
        hide_index=True,
