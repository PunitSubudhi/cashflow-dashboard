...
def render():
    ...
    tab1, tab2, tab3 = st.tabs(["📋 Upcoming Projections", "🔄 Recurrence Rules", "➕ Add Rule"])
...
def render_projections_view():
    ...
    earliest_rule_date = min((r.start_date for r in rules), default=date.today()) if rules else date.today()
    ...
    if not projections:
        st.info("No projected transactions for this period. Add recurrence rules to generate projections.")
        return
    
    # Count past due items
    past_due = [p for p in projections if p['date'] < date.today()]
    if past_due:
        st.warning(f"⚠️ {len(past_due)} projection(s) are past due and need to be marked as paid/received.")
    
    # Display projections
    data = []
    for i, proj in enumerate(projections):
        status = "⏰ Past Due" if proj['date'] < date.today() else ("📅 Today" if proj['date'] == date.today() else "🔮 Upcoming")
        data.append({
            'Index': i,
            'Status': status,
            'Date': proj['date'],
    ...
    if projections:
        ...
        if st.button("Mark as Paid/Received", type="primary"):
            ...
            # Create an actual transaction from the projection
            create_transaction(
...
def render_recurrence_rules():
    ...
    if not rules:
        st.info("No recurrence rules defined. Add a rule to start generating projections.")
        return
    ...
    if selected_id:
        ...
        if selected_rule:
            with st.form("edit_rule_form"):
                ...
                with col1:
                    edit_desc = st.text_input("Description", value=selected_rule.description)
                    edit_amount = st.number_input("Amount", value=selected_rule.amount, min_value=0.0)
                ...
                with col2:
                    edit_freq = st.selectbox(
                        ...
                        index=["DAILY", "WEEKLY", "MONTHLY", "YEARLY"].index(selected_rule.frequency)
                    )
                    edit_start = st.date_input("Start Date", value=selected_rule.start_date)
                    edit_end = st.date_input(
                        "End Date (optional)",
                        value=selected_rule.end_date if selected_rule.end_date else None
                    )
...
def render_add_rule_form():
    ...
    with st.form("add_rule_form", clear_on_submit=True):
        ...
        with col2:
            frequency = st.selectbox("Frequency", ["MONTHLY", "WEEKLY", "DAILY", "YEARLY"])
        ...
        submitted = st.form_submit_button("Add Recurrence Rule", type="primary", use_container_width=True)
