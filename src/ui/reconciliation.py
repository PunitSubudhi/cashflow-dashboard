    ...
    if uploaded_file:
        ...
        with col1:
            date_col = st.selectbox(
                ...
                if detection['date_candidates'] and detection['date_candidates'][0] in detection['all_columns']
                else 0
            )
        ...
        with col2:
            desc_col = st.selectbox(
                "Description Column",
                detection['all_columns'],
                index=detection['all_columns'].index(detection['description_candidates'][0])
                if detection['description_candidates'] and detection['description_candidates'][0] in detection['all_columns']
        ...
        with col3:
            amount_col = st.selectbox(
                ...
                if detection['amount_candidates'] and detection['amount_candidates'][0] in detection['all_columns']
        ...
        if st.button("Parse and Import", type="primary"):
            ...
            if error:
                ...
                st.success(f"Successfully imported {imported_count} transactions. Batch ID: {batch_id}")
...
def render_staging_area():
    ...
    for _, row in df.iterrows():
        ...
        with col1:
            if st.checkbox("Select", key=f"select_{row['ID']}", label_visibility="collapsed"):
    ...
    category = st.text_input(
        ...
        help=f"Existing categories: {', '.join(existing_categories)}" if existing_categories else ""
    )
    ...
    with col2:
        if len(selected_ids) >= 2:
            ...
            if st.button("Merge Selected", type="secondary"):
                if merge_desc:
                    result = merge_bank_imports(selected_ids, merge_desc, category=category)
                    if result:
                        st.success(f"Merged {len(selected_ids)} transactions into '{merge_desc}'!")
                        st.rerun()
                    else:
                        st.error("Failed to merge. Make sure all selected items are pending.")
                else:
