...
def render():
    ...
    with col2:
        st.metric(
            "Monthly Inflow",
            f"£{metrics['monthly_inflow']:,.2f}"
        )
    
    with col3:
        st.metric(
            "Monthly Outflow",
            f"£{metrics['monthly_outflow']:,.2f}"
        )
    ...
    tab1, tab2, tab3 = st.tabs(["📈 Cashflow Chart", "📋 Transactions", "➕ Add Transaction"])
...
def render_cashflow_chart():
    ...
    if not df.empty and 'balance' in df.columns:
        ...
        balance_chart = alt.Chart(df).mark_line(
            color='#1f77b4',
            strokeWidth=2
        ).encode(
        ...
        # Combine charts
        combined_chart = (balance_chart + today_line + today_label).properties(
            height=400
        ).interactive()
        ...
        st.info("No transaction data available. Add some transactions to see the chart.")
        ...
        if selected_txn:
            with st.form("edit_transaction_form"):
                ...
                with col1:
                    ...
                    edit_desc = st.text_input("Description", value=selected_txn.description)
                    edit_amount = st.number_input("Amount", value=selected_txn.amount, min_value=0.0)
                ...
                with col2:
                    ...
                    edit_category = st.text_input("Category", value=selected_txn.category or "")
                
                btn_col1, btn_col2 = st.columns(2)
                with btn_col1:
                    update_btn = st.form_submit_button("Update Transaction", type="primary")
                with btn_col2:
                    delete_btn = st.form_submit_button("Delete Transaction", type="secondary")
