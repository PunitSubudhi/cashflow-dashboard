...
def calculate_running_balance(
    ...
    actuals = get_all_transactions(status='ACTUAL', start_date=opening_date, end_date=end_date)
    ...
    for proj in projections:
        all_transactions.append({
            ...
            'amount': proj['amount'] if proj['type'] == 'INFLOW' else -proj['amount'],
            'type': proj['type'],
            'status': 'PROJECTED'
        })
...
def generate_projections(start_date: date, end_date: date, exclude_recorded: bool = True) -> List[Dict]:
    """
    Generate projected transactions from recurrence rules.
    ...
    if exclude_recorded:
        actuals = get_all_transactions(status='ACTUAL', start_date=start_date, end_date=end_date)
        for txn in actuals:
...
def expand_recurrence(rule: RecurrenceRule, start_date: date, end_date: date) -> List[date]:
    """
...
def get_summary_metrics(as_of_date: Optional[date] = None) -> Dict:
    ...
    if as_of_date.month == 12:
        month_end = as_of_date.replace(year=as_of_date.year + 1, month=1, day=1) - timedelta(days=1)
    else:
        month_end = as_of_date.replace(month=as_of_date.month + 1, day=1) - timedelta(days=1)
    
    monthly_txns = get_all_transactions(status='ACTUAL', start_date=month_start, end_date=month_end)
    ...
    projected_change = sum(
        p['amount'] if p['type'] == 'INFLOW' else -p['amount']
...
def get_category_breakdown(
    ...
    transactions = get_all_transactions(status='ACTUAL', start_date=start_date, end_date=end_date)
    ...
    data = [{'category': t.category or 'Uncategorized', 'amount': t.amount} for t in outflows]
